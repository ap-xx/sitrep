const CENTROIDS: Record<string, { lat: number; lng: number }> = {
  ...
  "Palestinian Territories": { lat: 31.9, lng: 35.2 },
  ...
  "Russian Federation": { lat: 61.5, lng: 105.3 },
  ...
  "Congo, Democratic Republic of the": { lat: -4.0, lng: 21.8 },
  "DR Congo": { lat: -4.0, lng: 21.8 },
  "Republic of the Congo": { lat: -0.2, lng: 15.8 },
  Congo: { lat: -0.2, lng: 15.8 },
};

export function getCountryCentroid(countryName: string): { lat: number; lng: number } | null {
  return CENTROIDS[countryName.trim()] ?? null;
}
